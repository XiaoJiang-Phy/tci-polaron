...
class TCIFitter:
    ...
    def _build_sweep_matrix_vectorized(self, d, left_indices, right_indices):
        ...
        # 1. 构造所有采样组合的索引网格
        # 我们需要生成 (r_prev, n_curr, r_next) 种组合
        # 使用广播机制生成全量索引矩阵 (r_prev, n_curr, r_next, N)
        total_samples = r_prev * n_curr * r_next
        
        # 2. 构造坐标大矩阵 (TotalSamples, N)
        # 这一步是核心：我们一次性拼凑出所有点的 N 维坐标
        all_coords = self._assemble_all_coords(d, left_indices, right_indices)
        
        # 3. 批量调用函数 (这是提速的关键！)
        # 假设 func 现在支持输入 (M, N) 形状的数组并返回 (M,)
        all_values = self.func(all_coords)
        
        # 4. 重新调整形状回 (r_prev * n_curr, r_next)
    ...
    #     # 构造最终的坐标数组容器
    #     # 形状为 (r_prev, n_curr, r_next, N)
    #     coords_tensor = np.zeros((r_prev, n_curr, r_next, self.n_dims))

    #     # 填充左侧维度 (从之前的路径继承)
    #     for i in range(d):
    #         # 索引通过 left_idx 广播
    #         # left_idx 形状为 (r_prev, d), 我们取第 i 列
    ...
    def _assemble_all_coords(self, d, left_idx, right_idx):
        ...
        # 1. 预分配大矩阵 (r_prev, n_curr, r_next, N)
        coords_tensor = np.zeros((r_prev, n_curr, r_next, self.n_dims))

        # 2. 填充左侧维度 (0 到 d-1)
        # 技巧：通过切片直接赋值。虽然有循环，但循环次数极少（仅 d 次）
        for i in range(d):
            # 这里的 [:, None, None] 就是利用广播机制，将 (r_prev,) 扩展为 (r_prev, 1, 1)
            # 然后自动填充到 coords_tensor 的 (r_prev, n_curr, r_next) 空间中
